...
const saleReportPage = async (req, res) => {
  try {
    ...
    res.render("saleReport", { orders: deliveredOrders });
  } catch (error) {
    res.status(400).status("Internal Server Error. Please try again later ");
  }
};
...
const downloadExcel = async (req, res) => {
  try {
    ...
    const orders = await fetchSaleReportData();
...
async function fetchSaleReportData() {
  try {
